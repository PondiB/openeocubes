...
datacube_crop <- p$load_collection(
  ...
  ,
  temporal_extent = c("2017-10-01", "2017-11-30"),
...
datacube_aoi <- p$load_collection(
  ...
  temporal_extent = c("2021-06-01", "2021-12-30"),
  bands = c("B02", "B03", "B04","B08")
)
...
start.time <- Sys.time()
openeo::create_job(result_predict)
openeo::list_results()
openeo::start_job("Insert the job ID here")
openeo::download_result("Insert the job ID here")
end.time <- Sys.time()
